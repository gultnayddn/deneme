...
// Survivor list external declaration (dosyan içinde survivors global değişkeni tanımlı olmalı)
extern SurvivorsList *survivors;
...
// Yeni fonksiyon: render_survivors()
void render_survivors(SDL_Renderer *renderer) {
    // Yardım bekleyenler (kırmızı)
    pthread_mutex_lock(&survivors->lock);
    Node *curr = survivors->head;
    while (curr) {
        Survivor *s = (Survivor *)curr->data;
        if (s->status == 0) {
            SDL_Rect rect = {s->coord.x * CELL_SIZE, s->coord.y * CELL_SIZE, CELL_SIZE, CELL_SIZE};
    ...
    // Yardım edilenler (yeşil)
    pthread_mutex_lock(&helpedsurvivors->lock);
    curr = helpedsurvivors->head;
    while (curr) {
        Survivor *s = (Survivor *)curr->data;
        SDL_Rect rect = {s->coord.x * CELL_SIZE, s->coord.y * CELL_SIZE, CELL_SIZE, CELL_SIZE};
...
// Eski draw_survivors fonksiyonu yorum satırına alındı çünkü artık render_survivors kullanılacak
/*
void draw_survivors() {
